...
try:
    ...
    def FetchIcon(guild_id, icon_hash):
        try:
            if not icon_hash:
                return None
    ...
    icon_hash = source_data.get("icon")
    if icon_hash:
        icon_data = FetchIcon(source_id, icon_hash)
        if icon_data:
            patch_data["icon"] = icon_data
            print(f"{SUCCESS} Icon fetched!", reset)
        else:
            print(f"{INFO} Could not fetch icon.", reset)
    ...
    if source_channels_response and source_channels_response.status_code == 200:
        source_channels = source_channels_response.json()

        categories = sorted(
            [c for c in source_channels if c.get("type") == 4],
        ...
        for category in categories:
            ...
            if resp and resp.status_code == 201:
                category_id_map[category["id"]] = resp.json()["id"]
                print(f"{SUCCESS} Created category:{red} {category.get('name', 'None')}", reset)
            else:
                print(f"{ERROR} Failed category:{red} {category.get('name', 'None')}", reset)
            time.sleep(0.5)
        ...
        for channel in other_channels:
            ...
            if channel.get("topic"):
                channel_data["topic"] = channel.get("topic")

            parent_id = channel.get("parent_id")
            if parent_id and parent_id in category_id_map:
                channel_data["parent_id"] = category_id_map[parent_id]
