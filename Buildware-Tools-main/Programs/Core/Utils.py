# Copyright (c) 2025-2026 JaMra — Buildware-Tools
...
github_repo_owner = "jmra2"
github_repo_name = "BuildWare-JaMra"

if sys.platform.startswith("win"):
    config_backup_dir = os.path.join(os.path.expanduser("~"), "AppData", "Roaming", "Buildware-Tools Configuration")
elif sys.platform.startswith("darwin"):
    config_backup_dir = os.path.join(os.path.expanduser("~"), "Library", "Application Support", "Buildware-Tools Configuration")
else:
    config_backup_dir = os.path.join(os.path.expanduser("~"), ".config", "Buildware-Tools Configuration")

config_backup_file = os.path.join(config_backup_dir, "Config.json")
...
def LoadData():
    ...
    return {
        ...
        "github_star"    : "",
        "page"           : 1,
        "plugins_visible": ["Example Plugin"],
    }
...
def CheckGithubStar():
    ...
    if open_tutorial in ["y", "yes"]:
        webbrowser.open("https://www.youtube.com/watch?v=soon")
...
def IsStargazer(username, headers):
    r = requests.get(
        f"https://api.github.com/user/starred/{github_repo_owner}/{github_repo_name}",
    ...
    target = f"{github_repo_owner}/{github_repo_name}".lower()
    page = 1
    while True:
        r = requests.get(
